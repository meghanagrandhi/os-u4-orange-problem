...
// All entries share the same directory prefix at the given depth.
// 'prefix' is the current directory path (e.g., "src/") — empty string for root.
static int write_tree_level(IndexEntry *entries, int count,
    ...
    while (i < count) {
        ...
        if (slash) {
            // It's in a subdirectory — get the directory name
            char dirname[256];
            ...
            // Build new prefix for this subdirectory
            char new_prefix[512];
            ...
            // It's a regular file — add directly
            TreeEntry *te = &tree.entries[tree.count++];
