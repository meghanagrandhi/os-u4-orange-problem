...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    // 2. Build header: "blob 42\0"  (the \0 is included via header_len)
    char header[64];
    int header_len = snprintf(header, sizeof(header), "%s %zu", type_str, len) + 1;

    // 3. Concatenate header + data into one buffer
 ...
 // 4. Hash the full object
    ...
    char shard_dir[512];
    strncpy(shard_dir, path, sizeof(shard_dir) - 1);
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
 ...
 // 3. Verify integrity — recompute hash and compare to expected
    ...
    *data_out = malloc(data_len + 1);
    ...
    ((char *)*data_out)[data_len] = '\0'; // null-terminate for safety
    *len_out = data_len;
